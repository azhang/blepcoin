...
---
Elrond is a blockchain and decentralized application platform that aims for 10k transactions per second, minimal latency, negligible fees, and interoperability. Elrond implements adaptive state sharding, which allows the network to scale linearly - the more validators, the more throughput and data it can support.
...
According to CryptoCalibur:

Hardcap: $4,750,000 (1.5M private, 3.25M public)

Private round unlocking schedule: 10% at TGE, 30% at 3, 6, and 9 months after TGE

Public round unlocking schedule: *unreleased*
...
**Consensus**: defines the SPOS - ~~Bellare-Neven consensus operation and chronology with rounds and epochs. The team says this will be changed to BLS multi-sig in the next few weeks,~~ reducing the necessary communication rounds from 5 to 2, and communication overhead from O(N^2) to O(N).
